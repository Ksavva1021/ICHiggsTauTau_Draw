...
if args.channel == "mm":
    ...
    gen_sels['tt_sel'] = '(gen_match_1>2&&gen_match_1<6 && gen_match_2>2&&gen_match_2<6)'
    gen_sels['ll_sel'] = '(gen_match_1==2&&gen_match_2==2)'
...
systematics['nominal'] = ('nominal','','(wt)',[],False)
...
while len(systematics) > 0:
    ...
    for index, systematic in enumerate(list(systematics.keys())[:max_systematics_per_pass]):
        ...
        for sample_name in data_samples:
            analysis.AddSamples('/vols/cms/ia2318/outputs/MSSM/2022_preEE_031023_withweights'+'/'+sample_name+'_zmm_2022_preEE.root', 'ntuple', None, sample_name)
